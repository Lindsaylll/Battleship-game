...
class Game (var dimension : Int, private var debug : Boolean) {
    ...
    fun attackCell(cell : Cell)  {
        // perform attack
//        println("activePlayer is ${activePlayer}")
...
//            if(ship.isSunk()){
//                listofSunkShip.add(ship.shipType.toString())
//            }
        }
        return cells
    }
    ...
    fun findSunkShip(): MutableList<String>{
        var listofSunkShip =  mutableListOf<String>()
        for( ship in boards[Player.Human]!!.placedShips){
            if(ship.isSunk()){
                listofSunkShip.add(ship.shipType.toString())
            }
        }
        return listofSunkShip
    }
}
